const ongoingActivityTab = document.getElementById('nav-ongoing');
const closedActivityTab = document.getElementById('nav-closed');

window.onload = pageLoad;

async function pageLoad() {
  const { data } = await axios.get('/api/v1/activity/my-activity');
  ...
  data.forEach((element) => {
    ...
    if (scheduleDate >= today) {
      ...
      closedActivity.push(element);
    }
  });
  insertCard(ongoingActivityTab, ongoingActivity);
  insertCard(closedActivityTab, closedActivity);
}

const insertCard = function (tab, content) {
